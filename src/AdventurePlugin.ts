...
export function pushAdministFunction(axios, getInfo: { endpoint: string; administratorId: any; token: any; guildId: any; defaultServer:any;}, getmessage){  //管理员推送事件处理函数
...
export const AdventurePlugin = (ctx: Context,getInfo: { endpoint: string; administratorId: any; token: any; functionList:any; guildId: any; defaultServer:any; }) =>{  //监听函数 
    // WebSocket连接配置
  ...
  ws.on('open', () => {  //连接成功
    ...
    let getmessage = `连接成功 \n`+
                     `开服监控：${getInfo.functionList.includes('开服监控')}\n`+
                     `新闻资讯：${getInfo.functionList.includes('新闻资讯')}\n`+
                     `游戏更新：${getInfo.functionList.includes('游戏更新')}\n`+
                     `贴吧速报：${getInfo.functionList.includes('贴吧速报')}\n`+
                     `关隘预告：${getInfo.functionList.includes('关隘预告')}\n`+
                     `云从预告：${getInfo.functionList.includes('云从预告')}\n`+
                     `${nowDate}`;
  ...
  function handleAdventureMessage(ctx: Context, message: any, getInfo: any) {  //message事件处理函数
    const serverStatus: Record<string, number> = {};  //定义对象存放开服维护信息（开服监控API）
    if (message.action === 2001 && getInfo.functionList.includes('开服监控')) {  //开服监控
      ...
      getInfo.guildId.forEach((Element: string,index) => {
        ...
        if (getInfo.defaultServer[index] === server) {
          axios.post(pushurl, pushmessage, { headers })
        }
      });
    }

    if (message.action === 2002 && getInfo.functionList.includes('新闻资讯')) {  //新闻资讯
    ...
    if (message.action === 2003 && getInfo.functionList.includes('游戏更新')) {  //游戏更新
    ...
    if (message.action === 2004 && getInfo.functionList.includes('贴吧速报')) {  //八卦速报
